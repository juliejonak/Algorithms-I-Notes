...
```
** In short: When adding different Big O notations, 
evaluate based on the one that has the worst time complexity, 
aka the one whose time output will affect the overall run time the most.
```
...
If this algorithm were still `O(n^2)`, the `j count` would equal 625.

But it's still running j `sqrt(n)` times for _every_ loop of i. While the loops are fewer, we'd still evaluate it like so:
...
But it's also a nested for loop which means the time complexity is `O(2 * n^2)`.
